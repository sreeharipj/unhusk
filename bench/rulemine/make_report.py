...
def sec_intro(ctx):
    ...
    w("never varied (V3), and 40 programs from a manifest curated by someone else for")
...
def sec_robustness(ctx):
    ...
    if e16 and "V4" in e16:
        ...
        w("**And a controlled corpus where the neighbourhood rules partly lose.** V4 is 40")
        w("programs from a manifest curated by someone else, for another purpose, sharing")
        w("no crate with anything else in this study — 80 builds, zero failures,")
        ...
        w("")
        w("**Here the trade is real rather than free.** R3 buys 1.96x the recall for 2.2")
        w("points of precision; R1 gains 0.8 pp of recall for 0.1 pp of precision. That is")
        w("a different picture from the main lockbox and from V3, and the reason is anchor")
        w("scarcity, now measurable rather than anecdotal: **28 of V4's 80 builds carry")
        w("fewer than 16 anchor-bearing functions**, against a main-corpus median of 31. A")
        w("+/-5 neighbourhood window cannot accumulate evidence that is not in the")
        w("neighbourhood.\n")
        w("So the scope condition in §2 is not a hedge. It is the summary of a corpus")
        w("selected to be unlike the one the rules were mined on, on which the rules")
        w("partly lose, and it is checkable at analysis time with no ground truth. §6.3")
        w("tests whether the moderating relationship it claims is actually there.\n")
    ...
    if e20 and e21:
        ...
        if ho:
            ...
            w("+25.93 pp.\n")
            w("![scope condition](figs/scope_light.png)\n")
            w("*Each point is one crate: its anchor count against how much more author")
            w("code the rule recovers than the incumbent does. Green bars are the median")
            w("advantage below and above 20 anchors; the blue line is the incumbent. The")
            w("two density rules slope up in every corpus; the caller rule is flat in all")
            w("three.*\n")
            extra = [(k, e21[k]) for k in ("V3 (codegen-units)", "V4 (fresh programs)")
                     if k in e21]
            if extra:
                w("**And it replicates on two corpora that played no part in proposing it")
                w("either.** Same test, same anchor definition:\n")
                w("| corpus | R3 | R1 | R2 |")
                w("|---|---|---|---|")
                for name, d in extra:
                    cells = []
                    for short in ("R3", "R1", "R2"):
                        v = d.get(short)
                        cells.append(f"rho {v['spearman_r']:+.3f}, p = {v['spearman_p']:.4f}"
                                     if v else "—")
                    w(f"| {name}, {extra and d.get('R3',{}).get('n','?')} crates | " + " | ".join(cells) + " |")
                w("")
                v4 = e21.get("V4 (fresh programs)", {}).get("R3")
                if v4:
                    w("The V4 row is worth one sentence of history, because it did not always")
                    w("say this. On the first half of that corpus (18 crates) the moderation was")
                    w("**not** significant — rho = +0.360, p = 0.143 — and `JOURNAL.md` recorded")
                    w("the non-replication at the time, before the second half existed, with the")
                    w("reading that it was underpowered rather than refuted. Doubling the corpus")
                    w(f"moved it to rho = {v4['spearman_r']:+.3f}, p = {v4['spearman_p']:.4f} with the coefficient essentially")
                    w("unchanged, which is the signature of a power problem rather than an absent")
                    w("effect. Above 20 anchors R3 wins")
                    w(f"{v4['high']['wins']}/{v4['high']['n']} of these fresh programs, median {v4['high']['median_pp']:+.2f} pp.\n")
            ...
            w("before either — and then confirmed twice more: R2's moderation is null on V3")
            w("(p = 0.26) and on V4 (p = 0.26) as well, while R1 and R3 are significant on")
            w("two of the three corpora. **Three corpora, three nulls for the caller rule,")
            w("against significant moderation for both density rules.** It is why R2 stays")
            w("in the proposed set despite being a null on aggregate held-out recall: it is")
            w("the rule for the regime the other two cannot serve.\n")
    ...
    w("Panel **b** is where the search happened, so it is in-sample; panel **c** is the")
    w("single held-out read and is the one the conclusion rests on. They are drawn side")
    w("by side rather than merged because the difference between them *is* a result:")
    w("the precision separation visible in b is gone in c, while the recall separation")
    w("survives. The second figure in §6.3 answers the question this one cannot — not")
    w("which operating point, but on which binary each rule is the right choice.\n")
    return o
...
def sec_close(ctx):
    ...
    w("   exist.")
    w("3. **The anchor-scarcity limit of §5.10 is measured in the middle and unmeasured")
    w("   at the bottom.** The moderating relationship is now confirmed on three corpora")
    w("   (§6.3), and R2's insensitivity to it with it, so the mechanism is no longer a")
    w("   hypothesis. What is still untested is the extreme: the sparsest build in any")
    w("   corpus here carries 2 anchor-bearing functions, while `blackcat_sphynx`")
    w("   carries 1 and two of the five wild samples carry none. Nothing measures the")
    w("   one-anchor regime, and the rule-choice guidance in §2 is an extrapolation")
    w("   below about 6 anchors. Closing that needs a corpus of deliberately tiny Rust")
    w("   programs, which is a build problem rather than an analysis one.")
    ...
    w("5. **The precision result did not replicate and the study says so, but that")
    ...
    w("6. **Fifteen held-out crates is not many for a clustered bootstrap.** The paired")
    ...
    w("7. **The precision floors are pooled.** A rule at 95% pooled precision is not at 95%")
    w("   in every crate; per-crate spreads are in `results/e11_lockbox.json` and the")
    w("   per-crate scatter is the first column of the §6.3 figure.")
    w("8. **One architecture, one OS, one object format.** x86-64 ELF. The PE port shares")
    w("   `classify.rs`/`xref.rs` and is expected to behave similarly, untested here.")
    w("9. **The composite rule of §6.4 is post-hoc and stays that way.** It is reported")
    w("   with numbers because hiding it would be worse, and it is not proposed.\n")
    ...
    if e21 and e21.get("test"):
        ...
        w("Then confirmed on two further corpora that played no part in proposing it: on")
        w("V3 and on 40 fresh programs, both density rules are significantly moderated and")
        w("**the caller rule is null on all three** (p = 0.49, 0.26, 0.26). The paper can")
        w("state the operating regime of each rule as a measured property rather than a")
        w("caveat, and can say which rule to reach for and why.\n")
