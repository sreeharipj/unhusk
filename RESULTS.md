...
| STRONG CLI/systems | ~98.2% | **97.9%** (n=379) | [95.9, 98.9] | [95.8, 99.2] |
...
[83.0, 91.4]). The sync stratum reads below the CLI-only 97.9% because it also contains
the macro/serde, crypto and TUI categories; CLI/systems alone is the 97.9% row.

Every row above is now emitted by `realval/report_results.py` — including the per-domain
breakdown that reproduces `docs/validation.md`'s own partition — so nothing in this table
is hand-computed.
...
*Generated by `realval/report_results.py` at 2026-07-17T01:03:15+05:30 over 32 provenance-PASSing binaries. Everything below is produced by script from binaries on disk — no hand-entered numbers.*
